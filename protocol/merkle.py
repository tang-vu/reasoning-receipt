...
Protocol core for `reasoning-receipt/1` (spec §6) and the `rr-trace/3`
reference schema. One Merkle root commits every leaf; anyone can prove
inclusion of a single leaf with a ~200-byte proof — without downloading
the full document.

Design notes:
- **Hash function:** SHA-256 throughout (Solidity has a `sha256` precompile,
  60 gas). Off-chain and on-chain agree on the byte interpretation.
- **Pair ordering:** sorted-pair concat (`sha256(min(a,b) || max(a,b))`).
  ...
  exact algorithm `ReceiptRegistryV2.verifyInclusion` implements on-chain.
- **Odd levels:** the lonely node is promoted (NOT duplicated).
  OZ-canonical, simpler proof generation, no second-pre-image risk.
- **Leaf format:** 32 raw bytes. Callers pass `bytes32` leaves.
"""
