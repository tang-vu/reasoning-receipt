...
function checkString(value, what) {
    ...
    for (const ch of value) {
        ...
        if (cp >= 0xd800 && cp <= 0xdfff) {
            throw new CanonError(`${what} contains an unpaired surrogate (U+${cp.toString(16).toUpperCase().padStart(4, "0")})`, E_NONCANONICAL);
        }
    }
}
...
function encodeNumber(value, out) {
    ...
    if (Math.abs(value) >= PORTABLE_ABS_BOUND) {
        throw new CanonError(`number ${value} outside portable range 2^53`, E_NONCANONICAL);
    }
...
function encode(value, out, depth) {
    ...
    if (value === null) {
        ...
        throw new CanonError(`unsupported type ${typeof value} is not canonicalizable`, E_NONCANONICAL);
    }
}
...
class Parser {
    ...
    value(depth) {
        if (depth > MAX_DEPTH)
            throw new CanonError("nesting exceeds limit", E_LIMIT);
        const c = this.peek();
        if (c === 0x22)
            return this.string();
        if (c === 0x7b)
    ...
    string() {
        ...
        while (true) {
            if (this.i >= this.s.length)
                throw new CanonError("invalid JSON: unterminated string", E_NONCANONICAL);
            const c = this.s.charCodeAt(this.i);
            if (c === 0x22) {
                this.i++;
                return out;
            ...
            if (c < 0x20)
                throw new CanonError("invalid JSON: raw control char", E_NONCANONICAL);
            if (c === 0x5c) {
                this.i++;
    ...
    object(depth) {
        ...
        while (true) {
            this.ws();
            if (this.peek() !== 0x22)
                throw new CanonError("invalid JSON: expected key", E_NONCANONICAL);
            const k = this.string();
            if (seen.has(k)) {
            ...
            this.ws();
            if (this.peek() !== 0x3a)
                throw new CanonError("invalid JSON: expected ':'", E_NONCANONICAL);
            this.i++;
            this.ws();
