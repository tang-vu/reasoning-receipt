...
import { CANONICALIZATION, SCHEMA_VERSION, verifyReceipt, } from "./receipt.js";
...
function bytes32(hexStr) {
    const raw = typeof hexStr === "string" && hexStr.startsWith("0x") ? hexStr.slice(2) : hexStr;
    return hexToBytes(raw);
}
export function legacyTraceHash(document) {
    return sha256Hex(legacyCanonicalBytes(document));
}
export function verifyTraceBlob(document, expectedHash) {
    ...
    for (const fieldName of ["market_id", "claim", "probability", "produced_at"]) {
        ...
        if (!checks[`has_${fieldName}`])
            errors.push(`missing_field: ${fieldName}`);
    }
    let recomputed = null;
    try {
...
function extractTrace3Nodes(document) {
    const out = {};
    ...
    if (Array.isArray(stances)) {
        for (const stance of stances) {
            if (stance === null || typeof stance !== "object" || Array.isArray(stance) || !("id" in stance))
                continue;
            const sdict = { ...stance };
            const evidenceList = sdict.evidence ?? [];
...
export function verifyTrace3(document) {
    const errors = [];
    const checks = {};
...
export function verifyReceiptDraft(document) {
    const errors = [];
    const checks = {};
    ...
    try {
        ...
        if (typeof subject === "string") {
            for (const ch of subject) {
                ...
                if (cp < 0x20 || cp === 0x7f)
                    throw new ReceiptError("subject contains a control character", "empty_subject");
            }
        }
    }
    catch (exc) {
    ...
    if (checks.unique_ids && rawNodes.length) {
        try {
            ...
            for (const nd of ordered) {
                recomputed[nd.id] = sha256Hex(legacyCanonicalBytes(nd));
            }
            const leaves = Object.keys(recomputed)
                .sort()
                .map((i) => bytes32(recomputed[i]));
            rootHex = merkleRoot(leaves);
            const embedded = (document.node_hashes ?? {});
...
export function verifyAny(document, expectedHash) {
